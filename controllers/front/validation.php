	...
	private function saveUserData($client_id, $payment_id, $user_id)
	{
		...
		try {
			...
			if ($count === 0)
			{
				...
				$sql = 'INSERT INTO `'._DB_PREFIX_.$table.'` (`clientId`, `paymentId`, `userId`) VALUES("'.$db->_escape($client_id).'", "'.$db->_escape($payment_id).'", '.(int)$user_id.');';
			}
			elseif ($count === 1)
			{
				if (Configuration::get('PIGMBH_PAYMILL_FASTCHECKOUT') === 'on')
				{
					...
					$sql = 'UPDATE `'._DB_PREFIX_.$table.'` SET `clientId`="'.$db->_escape($client_id).'", `paymentId`="'.$db->_escape($payment_id).'" WHERE `userId`='.(int)$user_id;
				}
				else
				{
					$this->log('Updated User $client_id.', var_export(array($client_id), true));
					$sql = 'UPDATE `'._DB_PREFIX_.$table.'` SET `clientId`="'.$db->_escape($client_id).'" WHERE `userId`='.intval($user_id);
				}
			}
